...
include_once 'language/zh_cn.php';
define('SELF', pathinfo(__FILE__, PATHINFO_BASENAME));
define('FCPATH', str_replace(SELF,'', __FILE__));
if(file_exists(FCPATH.'../data/install.lock')){
	$url = "http://".$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'];
	$url = substr($url,0,-17);
	echo $lang['havestep'].'<a href="'.$url.'">'.$url.'</a>';exit;
}
	...
	if(!empty($dbhost) && !empty($dbuser)){
		if(!@mysql_connect($dbhost,$dbuser,$dbpwd)) {
			...
			if(mysql_select_db($dbname)){
				...
				$str = file_get_contents($DatabasePath);
				$str = str_replace('$db[\'default\'][\'hostname\']=\'localhost\';','$db[\'default\'][\'hostname\']=\''.$dbhost.'\';',$str);
				$str = str_replace('$db[\'default\'][\'username\']=\'root\';','$db[\'default\'][\'username\']=\''.$dbuser.'\';',$str);
				$str = str_replace('$db[\'default\'][\'password\']=\'root\';','$db[\'default\'][\'password\']=\''.$dbpwd.'\';',$str);
				$str = str_replace('$db[\'default\'][\'database\']=\'x6cms\';','$db[\'default\'][\'database\']=\''.$dbname.'\';',$str);
				file_put_contents($DatabasePath,$str);
				$str = file_get_contents($ConfigPath);
				$str = str_replace('http://demo.x6cms.com/',$base_url,$str);
...
if(isset($dirfile)){
	foreach($dirfile as $item){
		if($item['status']==0){
			$purviewstr .= $item['path'].$lang['nowritepurivew'].'<br>';
		}else if($item['status']==-1){
			$purviewstr .= $item['path'].$lang['nohave'].'<br>';
		}
	}
	if($purviewstr!=''){
		?>
...
<table width="80%">
	...
	<tr><td><?php echo $lang['dbname'];?> </td><td><input type="text" name="dbname" id="dbname" value="x6cms" /></td></tr>
</table>
</td></tr>
...
<tr><td><input type="button" onclick="subOK()" value="<?php echo $lang['installx6cms'];?>" /><font color="red"><?php echo $error?></font></td></tr>
</table>
</form>
<?php 
}else{
